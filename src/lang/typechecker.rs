...
impl TypeChecker {
    ...
    pub fn run(&self, program: &SourceProgram) -> Result<usize, String> {
        ...
        program
            ...
            .iter()
            .try_for_each(
                |(array, (shape, _))| match store.insert(array, shape.len()) {
        ...
        match expr {
            ...
            Indexing(arr, index_list) => {
                ...
                if arr_dim >= num_indices {
                    ...
                    Err(format!(
                        "array with {} dimensions cannot have index list of length {}",
                        arr_dim, num_indices
                    ))
                }
            }
