...
class SEALWrapper:
    ...
    def mask(self, mask_vec: list[(int, int, int)]):
        ...
        while len(mask_vec) > 0:
            ...
            if mask_acc is None:
                lst = [1 if lo <= i and i <= hi else 0 for i in range(extent)]
                mask_acc = np.array(lst)

            else:
                lst = [mask_acc if lo <= i and i <= hi else np.zeros(mask_acc.shape) for i in range(extent)]
...
def server(wrapper):
    ...
    mask_1 = wrapper.mask([(32, 0, 29), (4, 0, 2), (32, 0, 29)])
    ...
    for i4 in range(3):
        instr1 = wrapper.rotate_rows((0 + (-128 * i4)), v_img_1.get())
