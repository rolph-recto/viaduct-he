...
def client_pre(wrapper):
    ...
    for i in range(16):
        cur_v_B_col = \
            wrapper.build_vector("B", None, [0, i], \
                [FilledDim(0, 16, 1, 0, 0, 0, 0)])
        wrapper.client_send("v_B_col{}".format(i), cur_v_B_col)
...
def server(wrapper):
    ...
    for i in range(16):
        cur_v_A1_row = \
            wrapper.build_vector("A1", None, [i, 0], \
                [FilledDim(1, 16, 1, 0, 0, 0, 0)]).get()
        v_A1_row.append(wrapper.encode_vec(cur_v_A1_row))

        cur_v_B_col = wrapper.server_recv("v_B_col{}".format(i)).get()
        v_B_col.append(cur_v_B_col)

    for i in range(16):
        row = []
        for j in range(16):
            cur_v_A2_point = \
                wrapper.build_vector("A2", None, [i, j], []).get()
            row.append(wrapper.encode_vec(cur_v_A2_point))

        v_A2_point.append(row)

    wrapper.start_server_exec()

    res = wrapper.ciphertext_array([16,16], 0)
    for i in range(16):
        for j in range(16):
            instr1 = wrapper.multiply_plain(v_B_col[j], v_A1_row[i])
            instr2 = wrapper.rotate_rows(-8, instr1)
            instr3 = wrapper.add(instr1, instr2)
            instr4 = wrapper.rotate_rows(-4, instr3)
            instr5 = wrapper.add(instr3, instr4)
            instr6 = wrapper.rotate_rows(-2, instr5)
            instr7 = wrapper.add(instr5, instr6)
            instr8 = wrapper.rotate_rows(-1, instr7)
            instr9 = wrapper.add(instr7, instr8)
            res.set([i,j], instr9)

    __out = wrapper.ciphertext_array([16,16], 0)
    for i in range(16):
        for j in range(16):
            for k in range(16):
                instr10 = wrapper.multiply_plain(res.get([k,j]), v_A2_point[i][k])
                instr11 = wrapper.add(__out.get([i,j]), instr10)
                __out.set([i,j], instr11)
