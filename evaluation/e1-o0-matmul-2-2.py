...
def client_pre(wrapper):
    ...
    v_B_1 = wrapper.build_vector("B", None, [0, 0], [FilledDim(0, 2, 1, 0, 0, 0, 0), FilledDim(1, 2, 1, 0, 0, 0, 0), EmptyDim(2, 0, 0, 0)])
...
def server(wrapper):
    ...
    v_A1_1 = wrapper.build_vector("A1", None, [0, 0], [FilledDim(1, 2, 1, 0, 0, 0, 0), EmptyDim(2, 0, 0, 0), FilledDim(0, 2, 1, 0, 0, 0, 0)])
    v_A2_1 = wrapper.build_vector("A2", None, [0, 0], [FilledDim(0, 2, 1, 0, 0, 0, 0), EmptyDim(2, 0, 0, 0), FilledDim(1, 2, 1, 0, 0, 0, 0)])
    ...
    instr3 = wrapper.rotate_rows(-4, instr2)
    wrapper.add_inplace(instr2, instr3)

    wrapper.set(res, [], instr2)
    __out = wrapper.ciphertext_array([], 0)
    instr7 = wrapper.multiply_plain(res.get(), v_A2_1.get())

    instr8 = wrapper.rotate_rows(-1, instr7)
    wrapper.add_inplace(instr7, instr8)

    wrapper.set(__out, [], instr7)
