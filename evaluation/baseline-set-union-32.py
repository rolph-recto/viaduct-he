...
def client_pre(wrapper):
    ...
    for i in range(32):
        cur_a_id = \
            wrapper.build_vector("a_id", None, [i, 0], \
                [FilledDim(1, 5, 1, 0, 1, 0, 0)])
        cur_a_data = \
            wrapper.build_vector("a_data", None, [i], [])

        cur_b_id = \
            wrapper.build_vector("b_id", None, [i, 0], \
                [FilledDim(1, 5, 1, 0, 1, 0, 0)])
        cur_b_data = \
            wrapper.build_vector("b_data", None, [i], [])

        wrapper.client_send("a_id{}".format(i), cur_a_id)
        wrapper.client_send("a_data{}".format(i), cur_a_data)
        wrapper.client_send("b_id{}".format(i), cur_b_id)
        wrapper.client_send("b_data{}".format(i), cur_b_data)
...
def server(wrapper):
    const_1 = wrapper.encode_vec(wrapper.const(1).get())
    const_neg1 = wrapper.encode_vec(wrapper.const(-1).get())
    n = 32
    ...
    b_data = []
    for i in range(n):
        cur_a_id = wrapper.server_recv("a_id{}".format(i)).get()
        cur_a_data = wrapper.server_recv("a_data{}".format(i)).get()
        cur_b_id = wrapper.server_recv("b_id{}".format(i)).get()
        cur_b_data = wrapper.server_recv("b_data{}".format(i)).get()
        a_id.append(cur_a_id)
        a_data.append(cur_a_data)
        b_id.append(cur_b_id)
        b_data.append(cur_b_data)

    wrapper.start_server_exec()

    a_sum = wrapper.ciphertext_array([], 0)
    for i in range(n):
        instr1 = wrapper.add(a_sum.get(), a_data[i])
        wrapper.set(a_sum, [], instr1)

    mask_pairs = wrapper.ciphertext_array([n,n], 0)
    for i in range(n):
        for j in range(n):
            instr2 = wrapper.subtract(a_id[j], b_id[i])
            instr3 = wrapper.multiply(instr2, instr2)
            wrapper.relinearize_inplace(instr3)
            instr4 = wrapper.multiply_plain(instr3, const_neg1)
            instr5 = wrapper.add_plain(instr4, const_1)
            instr6 = wrapper.rotate_rows(-4, instr5)
            instr7 = wrapper.multiply(instr5, instr6)
            wrapper.relinearize_inplace(instr7)
            instr8 = wrapper.rotate_rows(-2, instr7)
            instr9 = wrapper.multiply(instr7, instr8)
            wrapper.relinearize_inplace(instr9)
            instr10 = wrapper.rotate_rows(-1, instr9)
            instr11 = wrapper.multiply(instr9, instr10)
            wrapper.relinearize_inplace(instr11)
            mask_pairs.set([i,j], instr11)

    b_mask = wrapper.ciphertext_array([n], 0)
    for i in range(n):
        cur_len = int(n / 2)
        cur_masks = [mask_pairs.get([i, j]) for j in range(n)]
        while cur_len > 0:
            new_masks = []
            for j in range(cur_len):
                instr12 = wrapper.multiply(cur_masks[j], cur_masks[j+cur_len])
                wrapper.relinearize_inplace(instr12)
                new_masks.append(instr12)

            cur_len = int(cur_len / 2) if cur_len > 1 else 0
            cur_masks = new_masks

        assert(len(cur_masks) == 1)
        b_mask.set([i], cur_masks[0])

    b_sum = wrapper.ciphertext_array([], 0)
    for i in range(n):
        instr13 = wrapper.multiply(b_data[i], b_mask.get([i]))
        wrapper.relinearize_inplace(instr13)
        instr14 = wrapper.add(b_sum.get(), instr13)

    __out = wrapper.ciphertext_array([], 0)
    wrapper.set(__out, [], wrapper.add(a_sum.get(), b_sum.get()))
