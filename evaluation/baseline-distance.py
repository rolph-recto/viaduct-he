...
def client_pre(wrapper):
    ...
    v_point = wrapper.build_vector("point", None, [0], [FilledDim(0, 64, 1, 0, 0, 0, 0)])
    wrapper.client_send("v_point", v_point)
...
def server(wrapper):
    ...
    v_point = wrapper.server_recv("v_point").get()
    v_tests = []
    for i in range(64):
        vec = wrapper.build_vector( \
            "tests", \
            None, \
            [i,0], \
            [FilledDim(1, 64, 1, 0, 0, 0, 0)] \
        ).get()
        v_tests.append(wrapper.encode_vec(vec))

    wrapper.start_server_exec()

    __out = wrapper.ciphertext_array([64], 0)
    for i in range(64):
        instr1 = wrapper.subtract_plain(v_point, v_tests[i])
        instr2 = wrapper.multiply(instr1, instr1)
        wrapper.relinearize_inplace(instr2)

        instr3 = wrapper.rotate_rows(-32, instr2)
        instr4 = wrapper.add(instr2, instr3)
        instr5 = wrapper.rotate_rows(-16, instr4)
        instr6 = wrapper.add(instr4, instr5)
        instr7 = wrapper.rotate_rows(-8, instr6)
        instr8 = wrapper.add(instr6, instr7)
        instr9 = wrapper.rotate_rows(-4, instr8)
        instr10 = wrapper.add(instr8, instr9)
        instr11 = wrapper.rotate_rows(-2, instr10)
        instr12 = wrapper.add(instr10, instr11)
        instr13 = wrapper.rotate_rows(-1, instr12)
        instr14 = wrapper.add(instr12, instr13)
        wrapper.set(__out, [i], instr14)
